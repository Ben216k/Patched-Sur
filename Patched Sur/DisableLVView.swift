    ...
    var body: some View {
        VStack {
            Text("Disable Library Validation")
                .bold()
            Text("Since the installer checks to see if the update supports your Mac, Patched Sur needs to inject a dylib into it so that the installer doesn't care about the incompatibility. However, this can only be done with Library Validation off, so Patched Sur will quickly turn that off then restart your Mac, so then you can continue with updating.").padding()
