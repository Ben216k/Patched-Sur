...
struct FinishedView: View {
    var body: some View {
        VStack {
            Text("Finished!").bold()
            Text("The patcher has finished running! Now you can boot into the USB (hold option as soon as your mac turns on and select the yellow disk) then run the installer like normal. It does say \"Reinstall macOS\" but that only replaces the system data, not your data. Once that finishes, you can patch your kexts with the post install app that is now in your Applications folder.")
